...
fn merge(left_array: Vec<i32>, right_array: Vec<i32> ) -> Vec<i32> {
    ...
    while l < left_array.len() && r < right_array.len() {
        if left_array[l] <= right_array[r] { // note: the condition less than or equal makes this a stable algorithm
    ...
    if l < left_array.len() {
        while l < left_array.len() { // add tail of left list
            result_vector.push(left_array[l]);
            l += 1;
        }
    }
    else {
        while r < right_array.len() { // add tail of right list
            result_vector.push(right_array[r]);
            r += 1; 
        }
    }
