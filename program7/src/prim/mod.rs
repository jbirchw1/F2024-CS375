...
/// 
/// let my_graph = prim::build_weighted_directed_graph_from_stdin();
/// prim::print_shortest_path(source, destination, &my_graph);
/// ```
/// 
/// # Time Complexity
/// <div class="warning">This function operates in 
/// O(VlogV) time complexity.</div>
/// Note that this is only due to the call to prim(). 
...
pub fn print_shortest_path(
    ...
    let (distances, predecessors) = prim(graph, start);
...
/// Prim's algorithm for all shortest paths from a start vertex.
/// 
...
/// 
/// let my_graph = prim::build_weighted_directed_graph_from_stdin();
/// let distances, predecessors = prim::prim(&my_graph, source);
/// ``` 
/// 
/// # Time Complexity
/// <div class="warning">This function operates in 
